...
//   pcan_msg->channel_dlc =
//       UCAN_MSG_CHANNEL_DLC(channel, pcan_fd_len2dlc[pmsg->size]);
...
//   static const uint8_t pcan_fd_dlc2len[] = {0, 1,  2,  3,  4,  5,  6,  7,
...
//   pcan_brp = ((PCAN_STM32_SYSCLK_HZ / 1000000u) * pcur->brp) /
//              (pcan_device.can[channel].can_clock / 1000000u);
//   pcan_bitrate = (((PCAN_STM32_SYSCLK_HZ) / pcan_brp) /
...
//             (pcan_device.can[UCAN_CMD_CHANNEL(pcmd)].opt_mask &
//              UCAN_OPTION_ISO_MODE) != 0;
...
//       uint32_t total_ns =
//           pcan_device.can[i].tx_time_ns + pcan_device.can[i].rx_time_ns;
