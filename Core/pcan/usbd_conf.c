...
void OTG_HS_IRQHandler(void) { HAL_PCD_IRQHandler(&hpcd_usb); }

void OTG_FS_IRQHandler(void) { HAL_PCD_IRQHandler(&hpcd_usb); }
#endif
...
void HAL_PCD_MspInit(PCD_HandleTypeDef *pcdHandle) {
  ...
  if (pcdHandle->Instance == USB_OTG_HS) {
    /* Configure USB HS clock source */
    PeriphClkInitStruct.PeriphClockSelection = RCC_PERIPHCLK_USB;
    PeriphClkInitStruct.UsbClockSelection =
        RCC_USBCLKSOURCE_PLL3; // Use PLL3Q for 48MHz
    ...
    */
    ...
    GPIO_InitStruct.Alternate =
        GPIO_AF12_OTG2_FS; // H7 uses OTG2_FS for HS peripheral
    ...
    */
    ...
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    GPIO_InitStruct.Alternate = GPIO_AF10_OTG1_FS; // H7 uses OTG1_FS
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev) {
  ...
  if (pdev->id == DEVICE_HS) {
    ...
    hpcd_usb.Init.dev_endpoints = 8; // Support up to EP7 (we need EP3 for CH2)
    hpcd_usb.Init.speed = PCD_SPEED_FULL; // Using FS on HS peripheral (PB14/15)
    ...
    hpcd_usb.Init.dev_endpoints = 6; // H7 FS hardware limit is 6 (EP0-5)
  ...
  /* H7 FS Core has 1.25 KB (320 words) total USB RAM */
  /* RX FIFO: 128 words (512 bytes) */
  HAL_PCDEx_SetRxFiFo(&hpcd_usb, 128);

  /* TX FIFOs: Balanced for all endpoints */
  HAL_PCDEx_SetTxFiFo(&hpcd_usb, 0, 32); // EP0: 32 words (128 bytes)
  HAL_PCDEx_SetTxFiFo(&hpcd_usb, 1, 32); // EP1: 32 words (128 bytes) [CMD]
  HAL_PCDEx_SetTxFiFo(&hpcd_usb, 2, 64); // EP2: 64 words (256 bytes) [CH1]
  HAL_PCDEx_SetTxFiFo(&hpcd_usb, 3, 64); // EP3: 64 words (256 bytes) [CH2]
  /* Total words: 128 + 32 + 32 + 64 + 64 = 320 words (EXACTLY 1.25 KB) */
