...
#define USBD_MANUFACTURER_STRING "PEAK-System"

/* Product-specific configuration based on build mode */
#if defined(PCAN_PRO) || defined(PCAN_CLASSIC_MODE)
/* Classic CAN Mode - PCAN-USB Pro (2 channels) */
#define USBD_PID_HS 0x000D
#define USBD_PRODUCT_STRING_HS "PCAN-USB Pro"
#define USBD_INTERFACE_CAN_STRING_HS "PCAN-USB Pro CAN"
#define USBD_INTERFACE_LIN_STRING_HS "PCAN-USB Pro LIN"
#elif defined(PCAN_PRO_FD)
/* CAN-FD Mode - PCAN-USB Pro FD (2 channels) */
...
#define USBD_INTERFACE_CAN_STRING_HS "PCAN-USB X6 CAN"
#define USBD_INTERFACE_LIN_STRING_HS "PCAN-USB X6 LIN"
#else
#error                                                                         \
    "No PCAN variant defined! Define one of: PCAN_PRO, PCAN_PRO_FD, PCAN_FD, PCAN_X6"
#endif
...
uint8_t *USBD_ManufacturerStrDescriptor(USBD_SpeedTypeDef speed,
                                        uint16_t *length) {
  UNUSED(speed);
...
uint8_t *USBD_InterfaceStrDescriptor(USBD_SpeedTypeDef speed,
                                     uint16_t *length) {
  if (speed == 0) {
    USBD_GetString((uint8_t *)USBD_INTERFACE_CAN_STRING_HS, USBD_StrDesc,
                   length);
  } else {
    USBD_GetString((uint8_t *)USBD_INTERFACE_CAN_STRING_HS, USBD_StrDesc,
                   length);
...
static uint8_t *USBD_UserStrDescriptor(USBD_HandleTypeDef *pdev, uint8_t index,
                                       uint16_t *length) {
  ...
  switch (index) {
  case 6:
    USBD_GetString((uint8_t *)USBD_INTERFACE_LIN_STRING_HS, USBD_StrDesc,
                   length);
    break;
  default:
...
#if (USBD_LPM_ENABLED == 1)
static uint8_t *USBD_USR_BOSDescriptor(USBD_SpeedTypeDef speed,
                                       uint16_t *length) {
  UNUSED(speed);
